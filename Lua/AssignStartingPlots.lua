...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		amounts_of_resources_placed = table.fill(0, 35), -- Stores amounts of each resource ID placed. WARNING: This table uses adjusted resource ID (+1) to account for Lua indexing. Add 1 to all IDs to index this table.
		luxury_assignment_count = table.fill(0, 35), -- Stores amount of each luxury type assigned to regions. WARNING: current implementation will crash if a Luxury is attached to resource ID 0 (default = iron), because this table uses unadjusted resource ID as table index.
		luxury_low_fert_compensation = table.fill(0, 35), -- Stores number of times each resource ID had extras handed out at civ starts. WARNING: Indexed by resource ID.
...
function AssignStartingPlots:DetermineRegionTypes()
	...
	for this_region, terrainCounts in ipairs(self.regionTerrainCounts) do
		...
		-- Marsh check.
		elseif (marshCount >= areaPlots * 0.15) then
			table.insert(self.regionTypes, 9);
			--print("-");
			--print("Region #", this_region, " has been defined as a Marsh Region.");

		-- Mountains check.
		elseif (peaksCount >= areaPlots * 0.2) then
			table.insert(self.regionTypes, 10);
			--print("-");
			--print("Region #", this_region, " has been defined as a Mountainous Region.");
		elseif (peaksCount >= areaPlots * 0.1) and (peaksCount + hillsCount >= areaPlots * 0.25) then
			table.insert(self.regionTypes, 10);
			--print("-");
			--print("Region #", this_region, " has been defined as a Mountainous Region.");
...
function AssignStartingPlots:FindFallbackForUnmatchedRegionPriority(iRegionType, regions_still_available)
	...
	local iMostMarsh, iMostMountains = 0, 0;	-- New terrains
	...
	local bestMarsh, bestMountains = -1, -1;	-- New terrains
	...
	for loop, region_number in ipairs(regions_still_available) do
		...
		elseif iRegionType == 9 then -- Find fallback for Marsh priority
			if marshCount > iMostMarsh then
				bestMarsh = region_number;
				iMostMarsh = marshCount;
			end
		elseif iRegionType == 10 then -- Find fallback for Mountains priority
			if (2 * peaksCount) + hillsCount > iMostMountains then
				bestMountains = region_number;
				iMostMountains = (2 * peaksCount) + hillsCount;
			end
		end
	end
	...
	elseif iRegionType == 8 and bestHybrid ~= -1 then
		return bestHybrid
	elseif iRegionType == 9 and bestMarsh ~= -1 then
		return bestMarsh
	elseif iRegionType == 10 and bestMountains ~= -1 then
		return bestMountains
	end
		...
		if bNeedsCoastalStart == true then
			...
			if bNeedsRiverStart == true then
				...
				if iNumRegionPriority > 0 then
					--print("- - - - - - - needs Region Priority!"); priGetStartRegionPriorityListForCiv_GetIDsnt("-");
